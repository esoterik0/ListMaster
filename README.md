...
pythonw start.py
```
Name the script something like `LM.bat` or `LM.ps1` respectively, and make a shortcut to it, make sure to start it in the correct folder ex `pyapps` or whatever you chose. It can be tricky to make a shortcut to a batch file on windows, google has the answer; one needs to manually edit the shortcut so it points to `cmd /C LM.bat`, .ps1 uses the powershell executable instead.
## Features
...
References to other tables must be surrounded by {}, Page has a copy item function to help with this.
#### Formula
...
The title can be separate from the list or on top of the list. If the title is separate there are buttons to set or add to the title. The title can also be edited manually.
...
Save & Quit will save the table to the currently open collection, and close the dialog.

one can set a page, parse, save lists, set a new page parse, ..., over and over to get all the tables out of a pdf quickly.
#### Export
- Export list (txt)
